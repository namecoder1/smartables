...
export const ZoneLayout: React.FC<ZoneLayoutProps> = ({
  ...
  useEffect(() => {
    if (selectedId && trRef.current && stageRef.current) {
      ...
      if (selectedNode) {
        // Target the 'shape' sub-group so the Transformer only wraps the actual element,
        // not the control buttons, chairs, or badges.
        const shapeNode = selectedNode.findOne('.shape') || selectedNode;
          ...
          <Layer>
            ...
            <Group>
              ...
              <Transformer
                ...
                onTransformEnd={(e) => {
                  const shapeNode = trRef.current?.nodes()[0];
                  if (!shapeNode) return;

                  const scaleX = shapeNode.scaleX();
                  const scaleY = shapeNode.scaleY();

                  // The Transformer moves the shape sub-group to keep one edge fixed.
                  // We need to transfer this displacement to the parent Group.
                  const dx = shapeNode.x();
                  const dy = shapeNode.y();

                  // Reset sub-group transform
                  shapeNode.scaleX(1);
                  shapeNode.scaleY(1);
                  shapeNode.x(0);
                  shapeNode.y(0);
                  shapeNode.rotation(0);

                  const parentGroup = shapeNode.parent;
                  if (!parentGroup) return;

                  const tableId = parentGroup.id();
                  const table = tables.find(t => t.uniqueId === tableId);
                  if (!table) return;

                  // Convert local displacement to global, accounting for parent rotation
                  const rot = parentGroup.rotation() * Math.PI / 180;
                  const cos = Math.cos(rot);
                  const sin = Math.sin(rot);

                  const updates: Partial<TableInstance> = {
                    x: parentGroup.x() + dx * cos - dy * sin,
                    y: parentGroup.y() + dx * sin + dy * cos,
                    rotation: parentGroup.rotation(),
                  };
