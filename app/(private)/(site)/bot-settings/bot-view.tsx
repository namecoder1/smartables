...
import FaqSection, { FaqContent } from "@/components/private/faq-section";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!result.success) {
          if (result.error === "Non autorizzato") {
            router.push("/login");
            return;
          }
    ...
    <div >
      <div className="flex flex-col md:flex-row gap-6 items-start relative">
        <div className="flex flex-col gap-6 w-full md:w-72">
          <Card className="w-full py-0 lg:w-full shrink-0 space-y-6 border-r rounded-4xl border-border">
            <div className="w-full flex flex-col gap-2">
              <div className="flex flex-col h-auto bg-transparent p-2 border-b-2 w-full space-y-1 items-start">
                <button
                ...
                >
                  <FaWhatsapp className="h-5 w-5 mr-3 opacity-80" />
                  Profilo
                </button>
                <button
              ...
              <div className="space-y-2">
                <Card className="gap-2 bg-white shadow-none mx-2 py-0">
                ...
                <Card className="gap-0 bg-white flex-row shadow-none items-center justify-between px-4 py-4 mx-2 mb-2">
                  <h3 className="text-md font-semibold">Serve aiuto?</h3>
                  <FaqContent className="w-fit" variant='minimized' title="Qui" faqs={faqs} />
                </Card>

                <Card className="gap-2 bg-white shadow-none mx-2 mb-2 p-4 flex flex-col items-center justify-center text-center space-y-2">
        ...
        {/* Main Content Area */}
        <Card className="flex-1 w-full min-w-0 py-0 gap-0">
          {activeTab === "branding" && (
              ...
              {primaryLocation ? (
                ...
                <div className="py-20 flex items-center justify-center">
                  <div className="flex flex-col items-center gap-2">
                    <MessageCircleQuestion className="text-primary" size={40} />
                    <p className="text-muted-foreground text-lg">Nessuna sede trovata per la configurazione.</p>
                  </div>
                </div>
              )}
            </div>
