    ...
    <div className='mb-4 border rounded-lg bg-card'>
      <div className="flex flex-col gap-2 p-2 border-b md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center justify-between w-full gap-4">
          ...
          <div className="hidden md:block flex-1 text-center min-w-[200px] text-lg font-semibold">
            {label()}
          </div>

          <div className="flex items-center gap-1 bg-muted p-1 rounded-md">
            <Button
              variant={toolbar.view === 'month' ? 'outline' : 'ghost'}
              size="sm"
              onClick={() => goToView('month')}
              className={cn("h-7 px-2 text-xs", toolbar.view === 'month' && "bg-background text-foreground shadow-sm")}
            >
              Mese
            </Button>
            <Button
              variant={toolbar.view === 'week' ? 'outline' : 'ghost'}
              size="sm"
              onClick={() => goToView('week')}
              className={cn("h-7 px-2 text-xs", toolbar.view === 'week' && "bg-background text-foreground shadow-sm")}
            >
              Settimana
            </Button>
            <Button
              variant={toolbar.view === 'day' ? 'outline' : 'ghost'}
              size="sm"
              onClick={() => goToView('day')}
              className={cn("h-7 px-2 text-xs", toolbar.view === 'day' && "bg-background text-foreground shadow-sm")}
            >
              Giorno
            </Button>
          </div>
