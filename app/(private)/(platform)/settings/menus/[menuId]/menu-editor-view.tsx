...
export default function MenuEditorView({ menu: initialMenu, organizationId }: MenuEditorViewProps) {
        ...
        <div className="container flex h-14 max-w-screen-2xl items-center justify-between px-4">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs text-muted-foreground hidden sm:block">Gestisci il contenuto del menù</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {publicLocations.length > 1 ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="icon" title="Vedi menu pubblico">
                ...
                <DropdownMenuContent align="end">
                  {publicLocations.map((loc: any) => (
                    <DropdownMenuItem
                      key={loc.slug}
                      onClick={() => window.open(`${window.location.origin}/m/${loc.slug}/${menu.id}`, '_blank')}
                    >
                      {loc.name}
                    </DropdownMenuItem>
                  ))}
              ...
              <Button variant="outline" size="icon" onClick={() => copyPublicLink(menu.id)} title="Vedi menu pubblico">
                <Globe className="h-4 w-4" />
              </Button>
            )}
            ...
            <Button variant="ghost" size="icon" onClick={handleDeleteMenu} className="text-destructive hover:text-destructive hover:bg-destructive/10">
                  ...
                  <div key={category.id} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                      {category.items?.sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0)).map((item: any) => (
                        <div
                          key={item.id}
                          className="group relative flex flex-row overflow-hidden rounded-lg border bg-card text-card-foreground shadow-sm hover:shadow-md transition-shadow cursor-pointer hover:border-primary"
                      ...
                      <button
                        ...
                        className="flex flex-col py-4 items-center justify-center h-full min-h-[140px] rounded-lg border border-dashed hover:bg-accent/50 transition-colors gap-2 text-muted-foreground hover:text-foreground"
                      >
