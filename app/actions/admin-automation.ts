...
export async function manualPurchaseNumber(
  ...
  try {
    ...
    // 2. Purchase
    console.log(
      `[Admin] Manually purchasing ${location.telnyx_phone_number}...`,
    );
...
export async function manualMetaRegistration(locationId: string) {
  ...
  try {
    ...
    // 2. Add to WABA
    console.log(`[Admin] Adding ${cleanNumber} to Meta WABA...`);
...
export async function manualVoiceVerification(locationId: string) {
  ...
  try {
    ...
    // 2. Request Code
    console.log(
      `[Admin] Requesting Voice Code for ${location.meta_phone_id}...`,
    );
