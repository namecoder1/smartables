...
import { StarButton } from "../private/star-button"
...
const pathToDataMap: Record<string, PageData> = {
  ...
  "/areas-management": {
    title: "Le tue Mappe",
    path: { current: "Le tue Mappe" },
    description: "Gestisci la disposizione dei tavoli e le sale"
  },
  "/site-settings": {
    title: "Impostazioni & Dettagli",
    path: { current: "Impostazioni & Dettagli" },
    description: "Gestisci le impostazioni della tua sede"
  },
  "/connections": {
    title: "Connessioni",
    path: { current: "Connessioni" },
    description: "Gestisci le connessioni collegate del tuo negozio"
  },
  "/compliance": {
    title: "Modulistica & Attivazione",
    path: { current: "Modulistica & Attivazione" },
  ...
  "/manage": {
    title: "Gestisci",
    path: { current: "Gestisci" },
    description: "Gestisci l'intero servizio dalla tua dashboard admin."
  },
}
...
export default function PageTitle({ title, description, star }: PageTitleProps) {
  ...
  if (star === false) {
    return (
      <div className="hidden lg:flex lg:flex-col lg:items-start lg:justify-center pl-2">
        <div className="flex items-center gap-1.5">
          <nav className="flex items-center gap-1.5 text-sm font-medium text-white">
            {finalBreadcrumbs.map((crumb, index) => (
              <React.Fragment key={`${crumb.href}-${index}`}>
                {index > 0 && <ChevronRight className="w-3.5 h-3.5 text-white" />}
                {crumb.isLast ? (
